...
if (length(script_argument) != 1L) stop("Run this file with Rscript code/manuscript_figures.R.")
script_path <- gsub("~\\+~", " ", sub("^--file=", "", script_argument))
repo_root <- normalizePath(file.path(dirname(script_path), ".."), mustWork = TRUE)
figures_dir <- file.path(repo_root, "figures", "manuscript")
results_dir <- file.path(repo_root, "results", "manuscript_figures")
...
cat("Manuscript figures completed successfully. Figures: ", figures_dir, "\n", sep = "")
